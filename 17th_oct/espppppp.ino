...
// ==== Server settings ====
const char* serverHost = "192.168.0.111";   // Your PC/server IP (not localhost!)
const int serverPort   = 80;
const char* addPath    = "/electro_dashboard/add_data.php";

void setup() {
  Serial.begin(115200);  // Mega <-> ESP link (must match Mega Serial1 baud!)
  delay(200);
...
void loop() {
  while (Serial.available()) {
    ...
    if (c == '\n') {
      ...
      serialBuffer = "";  // reset buffer after one full line
    } else {
      serialBuffer += c;
    }
  }
